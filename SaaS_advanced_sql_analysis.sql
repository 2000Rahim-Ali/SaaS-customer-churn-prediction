...
-- Churn occurring within the first 6 months of tenure
SELECT
    COUNT(*) FILTER (WHERE tenure_months <= 6) AS churned_within_6_months,
    COUNT(*) AS total_churned_customers,
    ROUND(
        100.0 * COUNT(*) FILTER (WHERE tenure_months <= 6) / COUNT(*),
        2
    ) AS pct_churned_within_6_months
FROM customer_churn_clean
...
-- Month-by-month breakdown of early churn
SELECT
    tenure_months AS tenure_month,
    COUNT(*) AS churned_customers,
    ROUND(
...
--  Why is churn concentrated in the first six months — what onboarding/engagement failures explain this?
SELECT
    tenure_band,
